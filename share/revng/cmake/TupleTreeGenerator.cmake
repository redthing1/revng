...
function(tuple_tree_generator_impl)
  ...
  if(NOT "${GENERATOR_TYPESCRIPT_PATH}" STREQUAL "")
    list(APPEND NODE_MODULES_DEPS generate-node_modules)
  endif()
  ...
  add_custom_target(
    ...
    DEPENDS "${GENERATOR_SCHEMA_PATH}" ${LOCAL_GENERATED_HEADERS}
            ...
            ${EXTRA_TARGETS} ${NODE_MODULES_DEPS})
endfunction()
...
function(tuple_tree_generator_compute_generated_cpp_files SCHEMA_PATH
  ...
  execute_process(
    COMMAND
      ${REVNG_PYTHON_TTG_COMMAND}
      "${SCRIPTS_ROOT_DIR}/tuple-tree-generate-cpp-paths.py" "--impl"
...
# Allow callers to override how Python codegen scripts are executed (e.g. via
# `uv run --with ...`). Fall back to system python if unspecified.
if(NOT DEFINED REVNG_PYTHON_TTG_COMMAND)
  set(REVNG_PYTHON_TTG_COMMAND python3)
endif()
...
function(
  ...
  add_custom_command(
    COMMAND
      ${REVNG_PYTHON_TTG_COMMAND} "${SCRIPTS_ROOT_DIR}/tuple-tree-generate.py"
      docs ${SCALAR_TYPE_ARGS}
...
function(
  ...
  add_custom_command(
    COMMAND
      ${REVNG_PYTHON_TTG_COMMAND} "${SCRIPTS_ROOT_DIR}/tuple-tree-generate.py"
      cpp --namespace "${NAMESPACE}" --include-path-prefix
      "${INCLUDE_PATH_PREFIX}"
...
function(
  ...
  add_custom_command(
    COMMAND
      ${REVNG_PYTHON_TTG_COMMAND} "${SCRIPTS_ROOT_DIR}/tuple-tree-generate.py"
      jsonschema --output "${OUTPUT_PATH}" ${STRING_TYPE_ARGS}
...
function(
  ...
  add_custom_command(
    COMMAND
      ${REVNG_PYTHON_TTG_COMMAND} "${SCRIPTS_ROOT_DIR}/tuple-tree-generate.py"
      typescript --output "${OUTPUT_PATH}" --global-name "${GLOBAL_NAME}"
      ${INCLUDE_FILE_ARGS}
...
function(
  ...
  add_custom_command(
    COMMAND
      ${REVNG_PYTHON_TTG_COMMAND} "${SCRIPTS_ROOT_DIR}/tuple-tree-generate.py"
      python --output "${OUTPUT_PATH}" ${PYTHON_MIXINS_ARGS}
      ${STRING_TYPE_ARGS}
