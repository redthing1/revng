...
static cl::OptionCategory SDKIsolateCategory("SDK Isolate Options", "");
...
static cl::opt<std::string>
  Emit("emit",
       cl::desc("Output format for isolated functions: bc (default) or ll"),
       cl::value_desc("format"),
       cl::init("bc"),
       cl::cat(SDKIsolateCategory));

static revng::OutputPathOpt Output("o",
                                   cl::desc("Output .tar.gz containing one "
                                            "LLVM module per function"),
                                   cl::cat(SDKIsolateCategory));
...
int main(int argc, char *argv[]) {
  revng::InitRevng X(argc, argv, "", { &SDKIsolateCategory });
  applyResourceRoots();
  ...
  std::vector<MetaAddress> Selected =
    AbortOnError(revng::sdk::direct::parseFunctionEntries(Binary, EntryStrings));
  ...
  auto Writable = AbortOnError(MaybeOutput->getWritableFile(revng::ContentEncoding::Gzip));
  revng::GzipTarWriter Tar(Writable->os());
  ...
  for (const MetaAddress &Entry : Selected) {
    ...
    std::string Ext = (Emit == "ll") ? ".ll" : ".bc";
    std::string Name = "isolated/" + safeFilename(Entry) + Ext;
    Tar.append(Name, Buffer);
  }
